...
struct EntryFormView: View {
    ...
    init(entry: Entry?, onSave: @escaping (Entry) -> Void) {
        _fieldValue = State(initialValue: entry?.meatType ?? "")
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                Form {
                    Section("Meat type") {
                        TextField("Meat type", text: $fieldValue)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let entry = Entry(
                            id: entryID ?? UUID(),
                            meatType: fieldValue.isEmpty ? "Untitled" : fieldValue,
                            date: date,
                            note: note
                        )
                        onSave(entry)
